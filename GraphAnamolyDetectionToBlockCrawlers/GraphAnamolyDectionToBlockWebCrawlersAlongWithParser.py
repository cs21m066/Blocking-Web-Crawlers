...
foldername = "/usr/share/modsecurity-crs/graphAnamolyDetectionToBlockCrawlers/ParsedLogs/"
outfolder = "/usr/share/modsecurity-crs/graphAnamolyDetectionToBlockCrawlers/outputFiles/"
log_files = os.listdir(foldername)

#parsing logs
regexSearch = re.compile("(?P<ip>(.+)) (?P<unknown>(\\-)|(.+)) (?P<user>(\\-)|(.+)) \\[(?P<dateandtime>\\d{2}\\/[A-Za-z]{3}\\/\\d{4}:\\d{2}:\\d{2}:\\d{2} (\\+|\\-)\\d{4})\\] [\"](?P<request>(.+))[\"] (?P<statuscode>\\d{3}) (?P<bytessent>\\d+) ([\"](?P<referrer>(\\-)|(.*))[\"]) ([\"](?P<useragent>.*)[\"])")

inputLogFile = "/usr/share/modsecurity-crs/graphAnamolyDetectionToBlockCrawlers/logs/dummPortalAccess.log"
outputLogFile = "/usr/share/modsecurity-crs/graphAnamolyDetectionToBlockCrawlers/ParsedLogs/parsedLogs.csv"


frame = pd.DataFrame(columns=["ip","datetime", "request", "statuscode", "referrer", "useragent"])


with open(inputLogFile, "r") as logfile:
    for line in logfile:
        match = regexSearch.search(line)
        frame.loc[len(frame.index)] = [match.group("ip"),match.group("dateandtime"),match.group("request"),match.group("statuscode"),match.group("referrer"),match.group("useragent")]           
...
for row in frame["request"]:
    if row == "-":
        ...
        rm, req, st = row.split()
        req_method.append(rm)
        subtype.append(st)
        request.append(req)

frame["date"] = frame["datetime"].str[:11]
frame["time"] = frame["datetime"].str[12:]

frame["req_method"] = req_method
frame["request"] = request
frame["subtype"] = subtype

frame.to_csv(outputLogFile)
...
rearranged = ["subtype", "request", "ip", "useragent", "date", "time", "req_method", "statuscode", "referrer"]


#filtering logs
def getUsefulFilter(filename):
    unfilteredData = pd.read_csv(foldername + filename)
    onlyIITMData = unfilteredData.loc[(unfilteredData['req_method'] == "GET")]
...
with open(outfolder + "blockedIPs.txt", "w") as blockedIPs:
    threshold = 2
    for index, user in allUserDf.iterrows():
        if index < threshold:
            blockedIPs.write(user['ip']+" - \n")
            print("blocked "+user['ip'])
